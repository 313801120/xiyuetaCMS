<!--#Include File = "class_function/admin_functin2024.asp"-->
<!--#Include File = "class_function/class_export_data.asp"-->
<% 
dim adminLevelList:adminLevelList=",超级管理员,普通管理员,入库人员,审核人员,出库人员" 'admin管理员的权限列表' 
'管理员的权限列表''
...
dim excludeAdminIDLIst   '排除管理员在限制列表里'20250907
	 

function get_adminPermissionLits()
	dim c,splxx,s
	c=c & "设置|登录限制IP|编辑登录限制IP|设置IP限制|上传图片管理|生成sitemap|数据维护|模板库|备份恢复数据,"
	c=c & "会员|会员列表|添加会员|编辑会员|删除会员,"
	c=c & "管理员|管理员列表|添加管理员|编辑管理员|删除管理员|基本资料|修改密码,"
	c=c & "管理员角色|管理员角色列表|添加管理员角色|编辑管理员角色|删除管理员角色,"
	c=c & "网站栏目|网站栏目列表|添加网站栏目|编辑网站栏目|删除网站栏目,"
	c=c & "文章|文章列表|添加文章|编辑文章|删除文章,"
	c=c & "网站配置|网站配置列表|添加网站配置|编辑网站配置|删除网站配置,"
	c=c & "单页管理|单页管理列表|添加单页管理|编辑单页管理|删除单页管理,"
	c=c & "友情链接|友情链接列表|添加友情链接|编辑友情链接|删除友情链接,"
	c=c & "省市分类|省市分类列表|添加省市分类|编辑省市分类|删除省市分类,"
	c=c & "聊天管理|聊天管理列表|添加聊天管理|编辑聊天管理|删除聊天管理,"
	c=c & "订单|订单列表|添加订单|编辑订单|删除订单,"
	c=c & "后台日志|后台日志列表|删除后台日志," 
	c=c & "生成html管理|生成html管理列表|添加生成html管理|编辑生成html管理|删除生成html管理,"
	c=c & "xiyueta页|xiyueta页列表|添加xiyueta页|编辑xiyueta页|删除xiyueta页," 
	splxx=split("layout_page,layout_page_config,layout_table_button,layout_table_toolbar,layout_table_action,layout_listPage_form,layout_action,layout_field,layout_database,layout_database_table,layout_database_field,layout_cai,layout_cai_action",",")
	for each s in splxx
			if s<>"" then
					c=c & ""&s&"|"&s&"列表|添加"&s&"|编辑"&s&"|删除"&s&"," 
			end if
	next
	get_adminPermissionLits=c
end function

'根据权限显示内容'20250821
function aQX(s,levelList)
		if checkAQX(levelList) then  'userRs_level=1为超级管理员'
				aQX=s
		end if
end function
...
'检测权限'20250821
function checkAQX(levelList)
		checkAQX=false
		if instr(levelList,"|"&session("adminid")&"|")>0 or session("adminid")=-999 then  'userRs_level=1为超级管理员'
				checkAQX=true
		end if
end function 
'和上面一样，只是为了名称区分用'20251101
function checkEditAQX(levelList)
	  checkEditAQX=checkAQX(levelList)
end function

'检测管理员权限20230913' 如  checkAdminPower("财务人员")
' function checkAdminPower(title)    '不需要这个'
' 	dim splxx,s
' 	splxx=split(adminLevelList,",")
' 	s=splxx(userRs_level)
' 	if s=title then
...
function isAdminPermission(title) 
	dim splxx,isOK,s
	if session("adminid")=-999  then '为超级管理员直接可操作' userRs_level=1
	...
	for each s in splxx
		if s<>"" then
			if instr("|"& userRs_permission &"|", "|"& s &"|")>0 then
				' call echo(userRs_permission,s)
				isOK=true
				exit for
			end if
		end if
...
function checkAdminRule(title)
		if session("adminid")=-999  then  '为超级管理员直接可操作' userRs_level=1
				checkAdminRule=true
				exit function
		end if
		'在排除管理员id列表里'
		if instr(","&excludeAdminIDLIst&",", ","& session("adminid") &",")>0 then
				checkAdminRule=true
				exit function
		end if
		checkAdminRule=isAdminPermission(title) 
end function

'显示管理员权限，为false 则显示停止
function showCheckAdminRule(s)
    if checkAdminRule(s) = false then
        call die("没有【"& s &"】权限")
    end if
end function
...
function checkAdminLoginIPAllow(ip)
    dim content,splstr 
    content=readfile(adminDir & "Admin_Login_Allow_IP.dat","")
    splstr=split(content,chr(10))  'vbcrlf 换成chr(10) 传github再下载这里需要这么改'
    checkAdminLoginIPAllow=checkIPMatch(ip, splstr)
end function 
'检测当前IP是否被限制 20240719
 function checkIpLimitList(iplimitlist)  
 	dim splstr
    splstr=split(iplimitlist,chr(10))  'vbcrlf 换成chr(10) 传github再下载这里需要这么改'
    checkIpLimitList=checkIPMatch(getip(), splstr)
 end function 
' IP检测函数 20251101
' 参数: checkIP - 要检测的IP地址
'       ipList - IP列表（数组或逗号分隔的字符串）
' 返回: True - 匹配成功, False - 不匹配
Function checkIPMatch(checkIP, ipList)
    Dim i, ipPattern, ipParts, checkParts, j
    Dim matchFlag
    
    checkIPMatch = False
    
    ' 如果ipList是字符串，转换为数组
    If IsArray(ipList) = False Then
        ipList = Split(ipList, ",")
    End If
    ' 遍历IP列表
    For i = 0 To UBound(ipList)
        ipPattern = phpTrim(ipList(i))
        
        ' 跳过空字符串
        If ipPattern <> "" Then
            ' 如果完全匹配（没有通配符）
            If InStr(ipPattern, "*") = 0 Then
                If ipPattern = checkIP Then
                    checkIPMatch = True
                    Exit Function
                End If
            Else 
                ' 有通配符，需要分段匹配
                ipParts = Split(ipPattern, ".")
                checkParts = Split(checkIP, ".")
                
                ' IP地址必须是4段
                If UBound(ipParts) = 3 And UBound(checkParts) = 3 Then
                    matchFlag = True
                    ...
                    For j = 0 To 3
                        ...
                        Else
                            ' 精确匹配
                            If ipParts(j) <> checkParts(j) Then
                                matchFlag = False
                                Exit For
                            End If
                        End If
...
'添加系统日志'
function addSystemLog(item,msg)
	dim adminid:adminid=-1
	dim sql
	if isAddSystemLog=true and session("adminrule")<>"self" then
		item=replace(item,"'","''")  '处理的'
		msg=replace(msg,"'","''")  '处理的'
		' call openconn()  
		if session("adminid")<>"" then adminid=session("adminid")
			' call eerr("getTableList",getTableList())
		sql="insert into " & db_PREFIX & "SystemLog (item,msgstr,adminid,ip) values('" & item & "','" & msg & "'," & adminid & ",'" & getIP() & "')"

		' call echo("getTableList_Conn",getTableList_Conn(conn))
...
function getOnLineSelectValue(sType,sFocus)
	...
	for i=1 to ubound(splA)
		if splA(i)<>"" then
    		...
    		if instr(","&sFocus&",", ","&splB(1)&",")>0 then
    			if c<>"" then c=c &" , "
    			c=c & splB(0)  '改成多选的了 20250818'
    			' getOnLineSelectValue=splB(0)
    			' exit function
    		end if
    	end if
	next
	getOnLineSelectValue=c
end function
'显示在线修改select值'   如   showOnLineSelectHtml("stype-你好_0-不好_1-其它_2",1)
function showOnLineSelectHtml(sType,sFocus)

	sType=handleConfigFuHao(sType)

	dim splA,i,s2,splB,c,sel
	splA=split(sType,"-")
	sFocus=sFocus &"" '转字符类型'

  c="<select name="""& splA(0) &""" id="""& splA(0) &""" lay-filter="""& splA(0) &""">" 
	for i=1 to ubound(splA)
		if splA(i)<>"" then
    		s2=splA(i) & "_" & splA(i)
    		splB=split(s2,"_") 
    		sel=""
    		if unHandleConfigFuHao(splB(1))=sFocus then   '先处理符号'
    			sel=" selected"
    		end if
...
'处理符号'20240816
function handleConfigFuHao(c)
	c=replace(c,"\,","[特别 maoha 特别]")
	c=replace(c,"\_","[特别 xiahuaxian 特别]")
	c=replace(c,"\-","[特别 shuxian 特别]")
	handleConfigFuHao=c
end function
'解处理符号'20240816
function unHandleConfigFuHao(c)
	c=replace(c,"[特别 maoha 特别]",",")
	c=replace(c,"[特别 xiahuaxian 特别]","_")
	c=replace(c,"[特别 shuxian 特别]","-")
	unHandleConfigFuHao=c
end function
...
function TS_handleAllAction(sType,rs)
		...
		elseif sAction="userinfo" then '获得用户信息'   
				TS_handleAllAction=getUserInfo(rs("memberid"))
		elseif sAction="layoutpage" then '布局页ID
		  			rsx.open"select * from ["& db_PREFIX &"layout_page] where id="&rs("layout_page_id"),conn,1,1
		  			if not rsx.eof then
								TS_handleAllAction=rsx("pagename") & "(" & rsx("pagetablename") & ")"
						end if:rsx.close 
		elseif sAction="layoutconfig" then '布局页ID
		  			rsx.open"select * from ["& db_PREFIX &"layout_field] where id="&rs("layout_field_id"),conn,1,1
		  			if not rsx.eof then
								TS_handleAllAction=rsx("title") & "(" & rsx("fieldName") & ")"
						end if:rsx.close 
