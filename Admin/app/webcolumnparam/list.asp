...
If Request("act") = "list" Then  
    ...
    page = Request("page")
    stemp = "{""data"":[" 
    sql1 = "select * from ["& db_PREFIX &"webcolumnparam]" 
...
elseif request("act")="del" then
  conn.execute"delete from ["& db_PREFIX &"webcolumnparam] where id in("&request("id")&")"
...
elseif request("act")="onlineedit" then 
  title=request("title")
  id=request("id")
  rs.open"select * from ["& db_PREFIX &"webcolumnparam] where title='"& title &"' and id<>" & id,conn,1,1
  if not rs.eof then
    response.write "{""info"": ""名称存在"",""status"": ""n"",""title"": """&rs("title")&"""}"  
  else
    conn.execute"update ["& db_PREFIX &"webcolumnparam] set  title='"&title&"' where id="&id
    response.write "{""info"": ""修改成功"&id&""",""status"": ""y""}"
  end if:rs.close
  Response.end()
'通过或取消'
elseif request("act")="isthrough" then
    conn.execute"update ["& db_PREFIX &"webcolumnparam] set isthrough="&IIF(request("isthrough")="true",1,0) &" where id="&request("id")
