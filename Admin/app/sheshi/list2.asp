...
function getSheShiIdList(parentid,addSql)
    ...
    while not rs.EOF 
        if c<>"" then c=c & ","
        c=c & rs("id")
        s=getSheShiIdList(rs("id"),addSql)
        if s<>"" then
            c=c & "," & s
        end if
...
If Request("act") = "list" Then 
    ...
    for i=nStart to nEnd
        rs.open "select * from ["& db_PREFIX &"sheshi] where id="&splxx(i),conn,1,1
        if not rs.eof then
...
elseif request("act")="del" then
  conn.execute"delete from ["& db_PREFIX &"sheshi] where id="&request("id")
