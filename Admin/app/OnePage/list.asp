...
If Request("act") = "list" Then  
    ...
    page = Request("page")
    stemp = "{""data"":[" 
    sql = "select * from ["& db_PREFIX &"OnePage]" 
...
elseif request("act")="del" then
  conn.execute"delete from ["& db_PREFIX &"OnePage] where id="&request("id")
...
elseif request("act")="onlineedit" then 
  ...
  title=request("title")
  id=request("id")
  rs.open"select * from ["& db_PREFIX &"OnePage] where "& field &"='"& title &"' and id<>" & id,conn,1,1
  if not rs.eof then
    response.write "{""info"": ""字段存在"",""status"": ""n"",""title"": """&rs("title")&"""}"  
  else
    conn.execute"update ["& db_PREFIX &"OnePage] set  "& field &"='"&title&"' where id="&id
