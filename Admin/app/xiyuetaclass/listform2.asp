...
if request("act")="save" then
  ...
  if isTrue=true then
    addsql=" where parentid="& parentid &" and columnName='"& columnName &"'"
    if id<>"" then
      addsql=addsql & " and id<>"&id
    end if
    rs.open"select * from ["& db_PREFIX &"xiyuetaclass]"&addsql,conn,1,3
    if not rs.eof then
      msg="栏目名称已经存在"
    else
      if id="" then
...
function xiyuetaclassSubInput(parentid,focusId,focusParentId)
  ...
  while not rs.EOF  
    ...
    c=c & "<option value="""& rs("id") &""""& sel &">"& getSubxiyuetaclassSort(rs("parentid"),"")  & rs("columnName")&"</option>"    
    c=c & xiyuetaclassSubInput(rs("id"),focusId,focusParentId)    
  rs.movenext:wend:rs.close
  xiyuetaclassSubInput=c
end function
