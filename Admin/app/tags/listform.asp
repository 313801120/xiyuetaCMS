...
call openconn()  
dim msg,isTrue,addSql,id,title,isthrough,sortrank,bodycontent
...
if request("act")="save" then
  ...
  if isTrue=true then
    ...
    rs.open"select * from ["& db_PREFIX &"tags]"&addsql,conn,1,3
    if not rs.eof then
      msg="栏目名称已经存在"
    else
      if id="" then
        ...
        rs.close
        rs.open"select * from ["& db_PREFIX &"tags] where id="&id,conn,1,3
      end if  
      rs("title")=title 
...
'显示
elseif id<>"" then
  rs.open"select * from ["& db_PREFIX &"tags] where id="&id,conn,1,1
  if not rs.eof then 
    id=rs("id") 
