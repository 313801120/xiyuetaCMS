...
call openconn() 
dim num,page,stemp,sql1,sql,mysql,currentPage,perpage,page_count,i,n,sS,sHr,totalrec,id,title,stype
'网站栏目查询
If Request("act") = "list" Then  
    ...
    page = Request("page")
    stemp = "{""data"":[" 
    sql1 = "select * from ["& db_PREFIX &"articlecount]" 
    ...
    If Not rs.EOF Then
        ...
        While (Not rs.EOF) And(Not page_count = rs.PageSize)
           ...
           stype=rs("type")
           if stype="fabulous" then
              stype="点赞"
           elseif stype="view" then
              stype="查看"
           end if

	       stemp = stemp & "{""i"":""" & i & """,""id"":""" & rs("id") & """,""articleid"":""" & rs("articleid") & """,""type"":""" & stype & """,""count"":""" & rs("count") & """,""title"":""" & title & """,""updatetime"":""" & rs("updatetime") & """,""ip"":""" & rs("ip") & """,""addr"":""" & look_ip(rs("ip")) & """,""createtime"":""" & rs("createtime") & """}" &sHr & ""
...
elseif request("act")="del" then
  conn.execute"delete from ["& db_PREFIX &"articlecount] where id in("&request("id")&")"
...
elseif request("act")="onlineedit" then 
  title=request("title")
  id=request("id")
  rs.open"select * from ["& db_PREFIX &"articlecount] where title='"& title &"' and id<>" & id,conn,1,1
  if not rs.eof then
    response.write "{""info"": ""名称存在"",""status"": ""n"",""title"": """&rs("title")&"""}"  
  else
    conn.execute"update ["& db_PREFIX &"articlecount] set  title='"&title&"' where id="&id
    response.write "{""info"": ""修改成功"&id&""",""status"": ""y""}"
  end if:rs.close
  Response.end()
'通过或取消'
elseif request("act")="isthrough" then
    conn.execute"update ["& db_PREFIX &"articlecount] set isthrough="&IIF(request("isthrough")="true",1,0) &" where id="&request("id")
    ...
    table.render({
        ...
        cols: [
            [
               ...
               ,{ field: 'type',width:90, title: '类型', sort: false }
               ,{ field: 'title', title: '标题', sort: false }
               ,{ field: 'count',width:90, title: '统计数', sort: false }
