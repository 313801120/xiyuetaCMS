...
If Request("act") = "list" Then  
    ...
    If Not rs.EOF Then
        ...
        While (Not rs.EOF) And(Not page_count = rs.PageSize)
	       ...
	       stemp = stemp & "{""id"":""" & rs("id") & """,""isthrough"":""" & isthrough & """,""count"":""" & rs("count") & """,""title"":""" & jsonCL(rs("title")) & """,""ip"":""" &rs("ip") & """,""ipaddr"":""" &look_ip(rs("ip")) & """,""createtime"":""" & rs("createtime") & """,""updatetime"":""" & rs("updatetime") & """}" &sHr & "" 
...
elseif request("act")="del" then
  conn.execute"delete from ["& db_PREFIX &"articlesearch] where id in("&request("id")&")"
...
elseif request("act")="onlineedit" then 
  title=request("title")
  id=request("id")
  rs.open"select * from ["& db_PREFIX &"articlesearch] where title='"& title &"' and id<>" & id,conn,1,1
  if not rs.eof then
    response.write "{""info"": ""名称存在"",""status"": ""n"",""title"": """&rs("title")&"""}"  
  else
    conn.execute"update ["& db_PREFIX &"articlesearch] set  title='"&title&"' where id="&id
    response.write "{""info"": ""修改成功"&id&""",""status"": ""y""}"
  end if:rs.close
  Response.end()
'通过或取消'
elseif request("act")="isthrough" then
    conn.execute"update ["& db_PREFIX &"articlesearch] set isthrough="&IIF(request("isthrough")="true",1,0) &" where id="&request("id")
...
elseif request("act")="updateTags" then 
    dim splstr,s
    conn.execute"update ["& db_PREFIX &"articlesearch] set isthrough=0"  '统计数为0'
    ...
    for i=1 to 500
        ...
        for each s in splstr
            if s<>"" then
                rsx.open"select * from ["& db_PREFIX &"articlesearch] where title='"& s &"'" ,conn,1,3
                if rsx.eof then
                    rsx.addnew
                    rsx("count")=1
                    rsx("title")=s
    ...
    table.render({
        ...
        cols: [
            [
                ...
                , { field: 'title', title: '搜索名', minWidth: 100, sort: true }
                , { field: 'count', title: '统计数', width: 100, sort: true }
                , { field: 'createtime', title: '初次搜索时间', width: 180, sort: true }
                , { field: 'updatetime', title: '最后搜索时间', width: 180, sort: true }
                , { field: 'ip', title: 'IP', width:140, sort: true }
                , { field: 'ipaddr', title: 'IP地址', width:140, sort: true }
