...
call openconn()  
dim msg,isTrue,addSql,id,title,isthrough,count,bodycontent

id=request("id") 
title=request("title")                    '标题'
count=request("count")              '排序' 
...
if request("act")="save" then
  ...
  if isTrue=true then
    ...
    rs.open"select * from ["& db_PREFIX &"articlesearch]"&addsql,conn,1,3
    if not rs.eof then
      msg="栏目名称已经存在"
    else
      if id="" then
        ...
        rs.close
        rs.open"select * from ["& db_PREFIX &"articlesearch] where id="&id,conn,1,3
      end if  
      rs("title")=title 
      rs("count")=count 
...
'显示
elseif id<>"" then
  rs.open"select * from ["& db_PREFIX &"articlesearch] where id="&id,conn,1,1
  if not rs.eof then 
    id=rs("id") 
    title=inputCL(rs("title") )  
    count=inputCL(rs("count") )  
...
<form id="form1" name="form1" class="layui-form"  method="post" action="?act=save&id=<%=id%>">
  <div class="layui-form" lay-filter="layuiadmin-form-useradmin" id="layuiadmin-form-useradmin" style="padding: 20px 0 0 0;">
    ...
    <div class="layui-form-item">
      ...
      <div class="layui-input-inline">
        <input type="text" name="count" lay-verify="number" placeholder="请输入排序" autocomplete="off" class="layui-input" value="<%=count%>">
      </div>
    </div> 
