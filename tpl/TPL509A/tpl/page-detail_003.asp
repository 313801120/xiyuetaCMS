...
.sub-content-fluid<%=cssName%> .sub-about{
    padding: 15px;
    border: 1px solid #ddd;
}
.sub-content-fluid<%=cssName%> .sub-about .sub-title{
    border-bottom: 1px solid #e2e2e2;
    text-align: center; 
}
.sub-content-fluid<%=cssName%> .sub-about .sub-title h2{
    font-size: 24px;
    color: #000;
    padding-top: 15px;
    line-height: 1.5;
}
.sub-content-fluid<%=cssName%> .sub-about .sub-title p{
    font-size: 14px;
    margin-bottom: 15px;
    color: #888;
    overflow: hidden;
...
.sub-content-fluid<%=cssName%> .sub-info{
    font-size: 16px;
    line-height: 2;
    color: #191919;
    margin-top: 30px;
}
.sub-content-fluid<%=cssName%> .sub-link{
    padding: 15px 0px;
    border-top: 1px solid #e2e2e2;
    margin-top: 20px;
}
.sub-content-fluid<%=cssName%> .sub-link p{
    color: #000;
    font-size: 13px;
    line-height: 1.5;
    padding: 5px 0px;
}
.sub-content-fluid<%=cssName%> .sub-link p a{
    font-size: 13px;
}
.sub-content-fluid<%=cssName%> .sub-link p a:hover{
    color:  #0d62be;
            ...
            <div class="sub-content-right">
                ...
                <div class="sub-about">
                    <div class="sub-title">
                        ...
                        <p>
                            <span>
                                <i class="iconfont icon-time1"></i>发布时间：<%=createtime%>
                            </span>
                            <i class="iconfont icon-yanjing"></i>浏览次数：<%=views%>
...
<% 
    ...
    if rsx.eof then
        ...
        response.Write("<p>上一篇：<a href="""& getArticleUrl(rsx("id")) &""">"&uTitle& rsx("title") &"</a></p>")
    end if:rsx.close
    ...
    if rsx.eof then
        ...
        response.Write("<p>下一篇：<a href="""& getArticleUrl(rsx("id")) &""">"&uTitle& rsx("title") &"</a></p>")
    end if:rsx.close
    end if
    %>
