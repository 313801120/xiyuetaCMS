...
.container<%=cssName%> a:hover{color: #0d62be;}
...
.container<%=cssName%> .list-1{margin: -20px 0 0 -20px;}
.container<%=cssName%> .list-1 li{float: left;width: 33.333333%;}
.container<%=cssName%> .list-1 .wrap{margin: 20px 0 0 20px;background: -webkit-linear-gradient(top, #fff, #eee);
    background: -moz-linear-gradient(top, #fff, #eee);
    background: -o-linear-gradient(top, #fff, #eee);
    background: linear-gradient(top, #fff, #eee);}
.container<%=cssName%> .list-1 .img-cover{display: block;}
.container<%=cssName%> .list-1 .img-cover span{padding-top: 62%;}
.container<%=cssName%> .list-1 .text{border: 1px solid #ddd;border-top: 0;padding: 15px 25px;text-align: center;}
.container<%=cssName%> .list-1 .text h4{font-size: 14px;margin: 0;font-weight: normal;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;}
.container<%=cssName%> .list-1 .img-cover:hover span{transform: scale(1.8);}
.container<%=cssName%> .ct2-sd{float: left;width: 228px;}
.container<%=cssName%> .ct2-sd-right{float: right;}
.container<%=cssName%> .ct2-mn{float: right;width: 940px;}
.container<%=cssName%> .ct2-mn-left{float: left;}
...
.container<%=cssName%> .list-5 li a:before{content: '';position: absolute;left: 0;top: 50%;width: 0;height: 0;margin-top: -3px;border-top: 3px solid transparent;border-left: 4px solid #0d62be;border-bottom: 3px solid transparent;}
...
.container<%=cssName%> .list-1{margin: -10px 0 0 -10px;}
.container<%=cssName%> .list-1 li{width: 50%;}
.container<%=cssName%> .list-1 li .wrap{margin: 10px 0 0 10px;}
.container<%=cssName%> .list-1 .text{padding: 10px 15px;}
...
%>   
    ...
    <%rs.movenext:next:rs.close%>
        ...
        <div class="ct2-mn ct2-mn-left">
            ...
            <div class="mn-box">
                <ul class="list-1 clearfix<%=cssName%>">
...
if navId<>"" then 
    ...
    if sKeyword<>"" then addsql=addsql & " and title like '%"& sKeyword &"%'"
end if
if sKeyword<>"" then
    if addsql="" then
        addsql=" where "
...
%>
                    <li>
                        <div class="wrap">
                            <a href="<%=getArticleUrl(rs("id"))%>" class="img-cover">
