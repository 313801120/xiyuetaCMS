...
.container<%=cssName%> h1{font-size:2em;margin:.67em 0;}
...
.container<%=cssName%> .content-title{margin-bottom: 20px;text-align: center;}
.container<%=cssName%> .content-title h1{font-size: 20px;margin: 0;font-weight: normal;color: #0d62be;}
.container<%=cssName%> .content-title p{margin: 15px 0 0;font-size: 12px;color: #999;border-top: 1px dashed #ddd;padding-top: 15px;}
.container<%=cssName%> .content-title p span{padding: 0 10px;display: inline-block;}
.container<%=cssName%> .content-body{line-height: 1.8;}
.container<%=cssName%> .content-exp{padding: 10px 15px;font-size: 12px;background-color: #f5f5f5;margin-top: 20px;border-radius: 5px;}
.container<%=cssName%> .content-exp li{line-height: 2;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;}
...
.container<%=cssName%> .content-title{margin-bottom: 15px;}
.container<%=cssName%> .content-title p{margin-top: 10px;padding-top: 10px;}
.container<%=cssName%> .content-title p span{padding: 0 5px;}
.container<%=cssName%> .content-exp{margin-top: 15px;}}
...
<% 
    ...
    if rsx.eof then
        response.Write("<li><span>上一篇：</span><a href=""javascript:void(0);"">没有了</a></li>")
    else
        response.Write("<li><span>上一篇：</span><a href="""& getArticleUrl(rsx("id")) &""">"&uTitle& rsx("title") &"</a></li>")
    end if:rsx.close
    
    rsx.open"select * from " & db_PREFIX & "ArticleDetail where id>"& id & addsql &" order by id asc"  ,conn,1,1
    if rsx.eof then
        response.Write("<li><span>下一篇：</span><a href=""javascript:void(0);"">没有了</a></li>")
    else
        response.Write("<li><span>下一篇：</span><a href="""& getArticleUrl(rsx("id")) &""">"&uTitle& rsx("title") &"</a></li>")
    end if:rsx.close
    end if
    %>
